...
class GameAutoInit:
    ...
    def find_win(self, cla="LDPlayerMainFrame", winname="未定事件簿"):
    ...
    def find_pic(self, picname, ret=(0, 0, 1080, 1920), r=0.8):
        """
    ...
    def find_pic_all(self, picname, ret=(0, 0, 1080, 1920), r=0.7):
        """
        @description : 图像识别之模板匹配
        ---------
        @param : 需要匹配的模板名
    ...
    def find_pic_ex(self, path, ret=(0, 0, 1080, 1920), r=0.85):
        """
        @description : 图像识别之模板匹配
        ---------
        @param : 需要匹配的模板名
    ...
    def 点击图片(self, name, ret=(0, 0, 1080, 1920), times=2.5, r=0.8):
        pos = self.find_pic(name, ret, r)
        # print(pos)
        if pos[0] > 0:
            self.点击(pos, times)

    def 循环点击图片(self, name, ret=(0, 0, 1080, 1920), times=5.5, r=0.8):
        while True:
            pos = self.find_pic(name, ret, r)
            print(pos)
            if pos[0] > 0:
    ...
    def 截屏(self, ret=(0, 0, 1080, 1920)):
        return self.get_screenEx(ret[0], ret[1], ret[2], ret[3])

    def 截屏保存(self, ret=(0, 0, 1080, 1920)):
